...
const Mullinear = () => {
  ...
  const Cal = () => {
    ...
    console.log(cal_y);
    console.log(numgen, arr_x, arr_y)
    try {
      ...
      axios
        ...
        .then((res) => {
          ...
          if (res.data === "") {
            ...
            let jsons = JSON.parse(res.data.multicol);
            console.log(jsons)
            for (let i = 0; i < num_x; i++) {
              for(let j=0; j < numgen; j++){
                document.getElementById("rowx" + i + "item" + j).value = jsons.x[i][j];
              }
            }
            for(let i=0; i<numgen; i++){
  ...
  const resultttable = () => {
    ...
    return (
      <Main>
        ...
        <input type="button" value="generate" onClick={guideInp}></input>
